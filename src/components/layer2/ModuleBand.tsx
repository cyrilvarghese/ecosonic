    ...
    <div className="relative h-24 w-full rounded-[var(--radius-md)] bg-card/60">
      {sequence.map((m, i) => (
        <button
          ...
          aria-label={`Design ${m.mode} module`}
          className={`absolute top-2 bottom-2 flex cursor-pointer flex-col items-center justify-center rounded-[999px] text-[color:var(--n-800)] shadow-md transition-calm hover:brightness-105 ${
            selectedMode === m.mode ? 'ring-2 ring-[var(--accent-ink)]' : ''
          }`}
          style={{
